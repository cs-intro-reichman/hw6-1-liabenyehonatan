...
public class Runigram {
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		Color[][] flippedVartical = new Color[image.length][image[0].length];

		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[0].length; j++) {

				flippedVartical[i][j] = image[image.length - 1 - i][j];
			}
		}
		return flippedVartical;
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[0].length; j++) {
				Color lumColor = luminance(image[i][j]);
				grayimage[i][j] = lumColor;
			}
		}
		return grayimage;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				scaled[i][j] = image[i * h0 / height][j * w0 / width];
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		...
		if (image1.length != image2.length || image1[0].length != image2[0].length) {
			Color[][] fixedPic = scaled(image2, image1[0].length, image1.length);
		}

		int h = image1.length;
		int w = image1[0].length;

		Color[][] blended = new Color[h][w];
